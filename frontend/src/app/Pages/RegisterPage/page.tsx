    ...
    <div className="flex flex-col gap-10 items-center mb-20">
      <div className="flex flex-col items-center justify-center flex-1 p-6">
        ...
        <p className="text-center mb-8 text-base">
          Crie sua conta e comece sua <span className="font-bold text-purple">jornada de aprendizado!!</span>
        </p>
        <FormRegister />
        <p className="mt-6 text-sm">
          Já tem uma conta? <a href="/login" className="font-bold hover:underline">Faça Login</a>
        </p>
      </div>
    </div>
  );
...
async function postUsuario(nome: string, email: string, senha: string) {
  try {
    ...
    if (!response.ok) {
      ...
      console.log("De alguma forma essa poha funcionou, parabéns Isaac por fazer o mínimo!");
    }
  } catch (error) {
...
async function getUsuarios(email: string) {
  try {
    ...
    if (!response.ok) {
      ...
      console.log("Já tem uma conta criada com esse teu email aí parça, lute!", data);
      return data;
    }
  } catch (error) {
    console.error("Erro:", error);
